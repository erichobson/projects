...
int main(){
  ...
  struct Fair   myFair;
  struct Guest  guestArray[TOTAL_GUESTS];

  // create pointers to structs
  struct Fair   *FairPtr = &myFair;
  struct Guest* guestPtrArray[TOTAL_GUESTS];
  struct Ride*  ridePtrArray[TOTAL_RIDES];
  ...
  // initialize each element of PtrArray(s) to point to corresponding element of Array(s).
  for(i = 0; i < TOTAL_GUESTS; i++) guestPtrArray[i] = &(guestArray[i]);
  for(i = 0; i < TOTAL_RIDES; i++) ridePtrArray[i] = &(myFair.fairRides[i]);

  // populate guestPtrArray using populateGuest() function
  for(i = 0; i < TOTAL_GUESTS; i++) populateGuest(guestPtrArray[i]);

  // populate ridePtrArray using populateRide() function
  for(i = 0; i < TOTAL_RIDES; i++) populateRide(ridePtrArray[i], i);

  // simulate admitting guest to the fair using admitGuest() function
  for(i = 0; i < TOTAL_GUESTS; i++) guestPtrArray[i]->admitted = admitGuest(guestPtrArray[i], FairPtr);
  ...
  for(i = 0; i < TOTAL_GUESTS; i++){
    ...
    if(r < 5){
      ...
      while(s >= 0){
        g = purchaseTickets(guestPtrArray[i], FairPtr, ticketBundles[s]);
        s -= 1;
        if(g == 1){
      ...
      if(g == 0){
        ...
        while(q > 0){   // iterates down by 1 until guest successfully purchases q ticket(s)
          g = purchaseTickets(guestPtrArray[i], FairPtr, q);
          if(g == 1){
            break;
          }else{
            q -= 1;
    ...
    // 50% chance to purchase pass
      ...
      g = purchasePass(guestPtrArray[i], FairPtr);
  ...
  for(i = 0; i < TOTAL_GUESTS; i++){
    if(guestPtrArray[i]->admitted == 0){
      printf("Guest #%3d: NOT ADMITTED\n", i);
    }else{
      if(guestPtrArray[i]->passOwned == 1){
        printf("Guest #%3d, Age: %2d, Height: %3dcm, Money: $%6.02f, Tickets:PASS, Rides:%3d\n", 
              i, guestPtrArray[i]->age, guestPtrArray[i]->height, guestPtrArray[i]->money, guestPtrArray[i]->numberOfRides);
      }else{
      printf("Guest #%3d, Age: %2d, Height: %3dcm, Money: $%6.02f, Tickets:%4d, Rides:%3d\n", 
              i, guestPtrArray[i]->age, guestPtrArray[i]->height, guestPtrArray[i]->money, guestPtrArray[i]->ticketsOwned, guestPtrArray[i]->numberOfRides);
      }
    }
  }
  ...
  while(h > 0){
    for(i = 0; i < TOTAL_GUESTS; i++){
      s = rand() % 5;   // random variable used to choose from ridePtrArray
      if(guestPtrArray[i]->age <= 12){
        // guest is < 13 years old, so selects random ride from first 5 elements 
        // of ridePtrArray using initial random variable between 0-4

        // if guest has no ticekts and no pass - skip
        if(guestPtrArray[i]->ticketsOwned == 0 && guestPtrArray[i]->passOwned == 0){
          continue;
        
        // else run attemptRide()
        }else attemptRide(guestPtrArray[i], ridePtrArray[s]);  

      }else if(guestPtrArray[i]->age > 12){
        // guest is > 12 years old, so selects random ride from last 5 elements
        // of ridePtrArray using initial random variable (+5) between 5-9
        s += 5;
